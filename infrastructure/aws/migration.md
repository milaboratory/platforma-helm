...
| Target cluster ready | Cluster Autoscaler, Kueue + AppWrapper, EFS CSI, EBS CSI, S3 IRSA all working. See [advanced-installation.md](advanced-installation.md) Steps 1–8. |
...
This is the **consistent dump you upload and restore** — nothing is mid-flight. The dump needs the instance running, but `--cancel-running-blocks` exited the service, so **start the source service again first** (keep users and submitters paused — the cancelled blocks will not resume and no new work can arrive). Then dump through the same endpoint as in Step 0.3.1:
...
The Batch job in Step 0.6.5 generates its own manifest and can copy existing objects without a bucket-level replication configuration. The reason to put one on anyway is to carry **live writes** to the destination:

* In the **full-freeze** strategy it is an optional safety net — if anything accidentally writes to the supposedly-frozen source bucket during the migration window, those writes propagate to the destination instead of being orphaned. Skip it only if you are certain no writes can occur.
* In the **online** strategy it is **required, not optional**, and must be applied *before* the backfill job (Step 0.6.5): it is the mechanism that streams the writes users make during the no-freeze window to the destination, so that by cutover only a tiny delta remains. Keep versioning enabled until cutover is complete.
...
Standard install — see [README.md](README.md) and [advanced-installation.md](advanced-installation.md) Step 10. The chart adopts the pre-existing PVC and StorageClass and starts the Platforma server. On first start the server applies any in-binary schema migrations against the restored DB.
