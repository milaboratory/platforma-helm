...
| Pinned production | a chart release tag like `v3.3.10` — append `&cloudshell_git_branch=v3.3.10` to the URL |
| Power user fork | their own branch |

Chart release tags are pushed to `platforma-helm` automatically by the
backend release pipeline whenever a new platforma version cuts (one tag
per chart version, matching `charts/platforma/Chart.yaml: version`). The
GCP Infrastructure Manager bundle ships under the same tag — no separate
