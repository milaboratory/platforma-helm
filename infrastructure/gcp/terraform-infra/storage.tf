...
resource "google_storage_bucket" "primary" {
  ...
  # Default false — primary storage holds user project data. tofu destroy
  # leaves the bucket in place; users must delete it manually if they really
  # want the data gone (matches AWS S3 retention pattern in the CloudFormation
  # runbook). Flip to true via var.gcs_force_destroy only for ephemeral
  # dev/test deployments where bucket contents are disposable — empty buckets
  # delete cleanly regardless, this just covers the non-empty case.
