...
Leaving `auth.providers` empty (the default) keeps the legacy blocks working
exactly as before, so an existing release is unaffected until it opts in.
  ...
  providers:
    - name: entra
      ...
      sso:
        ...
        userIdClaim: oid           # Entra's stable subject
        redirectPorts: [8765]
        clientSecret:              # confidential clients only
          secretName: entra-oauth
      ...
      map:
        ...
        displayName: name
        groups: groups
      # Match on email so a login adopts an existing record carrying that
      # address. READ THE CAVEAT BELOW before setting this on an instance that
      # already has users.
      lookUpAttr: email
      trustUnverifiedEmail: true   # Entra sends no email_verified claim
      createUsers: true
      roles:
    ...
    - name: local                  # break-glass, alongside the SSO provider
      ...
      htpasswd:
        credentials:
          - { username: breakglass, password: <password> }
      adminUsers: ["breakglass"]
```
...
The one setting the chart cannot mount is the LDAP bind password, because
`auth.ldap.bind-password` takes the secret as a flag value rather than a file.
Set `ldap.bindPassword` for a non-production directory, or inject the flag's
environment variable from a Secret whose value carries the provider id:
  ...
  env:
    secretVariables:
      - name: PL_AUTH_LDAP_BIND_PASSWORD
        ...
        secretKey: bind-password     # value must be "<providerId>=<password>"
```

### Caveat: `lookUpAttr: email` keeps the adopted record's login

Matching on email is what lets a login claim an account it did not create — a
user seeded by CSV provisioning, or one recorded under a provider that is no
longer configured. Adoption deliberately **keeps that record's stored login**,
so the provider's own `map.login` value is not written over it (otherwise the
next provisioning run would miss the row and mint a duplicate).

That matters when cutting over from the legacy `--sso-idp-*` flags, which always
stored the login as the OIDC `sub` regardless of `--sso-idp-user-id-claim`. The
first login under the new provider adopts the old record and inherits that
`sub`, so the Desktop App shows an opaque string instead of an address. It
resolves itself: adoption indexes the record by email, so the *next* login is a
point lookup, `KeepStoredLogin` is false, and the stored login is refreshed to
the mapped value.

If you would rather not depend on that, leave `lookUpAttr` unset. The match key
is then the login (`map.login`), which provisions a clean record straight away —
at the cost of orphaning grants and projects owned by the old records. Note that
`trustUnverifiedEmail` must be dropped in the same change: boot validation
rejects it unless `lookUpAttr` is `email`.

Full field reference and defaults: the commented `auth.providers` example in
`values.yaml`.

## Cleaning up duplicate accounts

Running several providers makes duplicates possible by construction: a
misconfiguration, or an identity that could not be matched, gives one person two
accounts. The spare is not harmless — it shows up in the sharing user picker, and
the projects in its root keep taking part in deduplication.

`pl-cli admin delete-user` removes one. It requires an explicit decision about the
projects when the account owns any, since neither default is safe to assume:

```bash
# Re-home every project to the account being kept, then delete the duplicate.
npx @platforma-sdk/pl-cli@latest admin delete-user <DUPLICATE_LOGIN> \
  --address <SERVER> \
  --admin-user <ADMIN> --admin-password <PASS> \
  --move-projects-to <LOGIN_TO_KEEP>

# Or delete the account and its projects together.
npx @platforma-sdk/pl-cli@latest admin delete-user <DUPLICATE_LOGIN> \
  --address <SERVER> \
  --admin-user <ADMIN> --admin-password <PASS> \
  --delete-projects
...
- `--move-projects-to` re-attaches the same project resources instead of copying
  them, so nothing is duplicated; a name the target already uses is suffixed rather
  than overwritten.
- The command prompts with the affected project list before acting. `--force` skips
  the prompt for scripted runs.
- Deletion is irreversible and takes the account's grants, identity-index entries
  (its login, its email and any alternative of either) and root resource with it. It
  also **frees those values** — the next sign-in under any of them creates a clean
  account rather than reviving the deleted one, which is what makes a duplicate
  resolvable once the provider config is fixed.
- Admin or controller credentials are required, and the credentials' own account
